...
def apply_cell_formatting(cell, value, cell_type: str = "text", is_special_row: bool = False, 
    ...
    Args:
        ...
        cell_type: Type de formatage ("text", "number", "time", "header")
    ...
    for i, (_, row) in enumerate(data_df.iterrows()):
        ...
        for j, header in enumerate(headers, start=start_col):
            ...
            if j == 1:  # Première colonne (généralement nom)
                cell_type = "text"
            elif j == 2 and 'Time' in header:  # Colonne de temps
                cell_type = "time"
            elif header.startswith('Rel. Area') or 'Relative Area' in header or 'RetentionTime' in header:
                cell_type = "number"
            else:
                cell_type = "text"
