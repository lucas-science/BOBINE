...
def standardize_injection_time(time_value):
    """
    Standardise les temps d'injection pour extraire uniquement HH:MM

    Args:
        time_value: Valeur de temps dans différents formats possibles

    Returns:
        str: Temps au format HH:MM ou chaîne vide si invalide
    ...
    try:
        ...
        if '-' in time_str and ' ' in time_str:
            ...
            if len(parts) >= 2:
                ...
                # Extraire seulement HH:MM (ignorer les secondes)
                if ':' in time_part:
        ...
        # Cas 3: Format déjà correct "HH:MM:SS" ou "HH:MM"
            ...
            # Extraire seulement HH:MM (ignorer les secondes si présentes)
...
def create_time_sort_key(time_str):
    """
    Crée une clé de tri pour les temps au format HH:MM ou HH:MM:SS

    Args:
        time_str: Chaîne de temps au format HH:MM ou HH:MM:SS
...
def calculate_total_time_duration(first_time_str, last_time_str):
    """
    Calcule la durée totale entre deux temps au format HH:MM

    Args:
        first_time_str: Premier temps au format HH:MM
        last_time_str: Dernier temps au format HH:MM

    Returns:
        str: Durée totale au format HH:MM ou "n.a." si erreur
    """
    try:
        from datetime import datetime, timedelta

        # Support HH:MM et HH:MM:SS
        if str(first_time_str).count(':') == 1:
            first_time = datetime.strptime(str(first_time_str), '%H:%M').time()
        else:
            first_time = datetime.strptime(str(first_time_str), '%H:%M:%S').time()

        if str(last_time_str).count(':') == 1:
            last_time = datetime.strptime(str(last_time_str), '%H:%M').time()
        else:
